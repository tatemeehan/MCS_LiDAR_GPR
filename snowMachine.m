...
MV = [ones(length(lidarGPR.Density(:)),1),LiDAR.A.A(kd.ix),LiDAR.A.northness(kd.ix),LiDAR.A.slope(kd.ix),LiDAR.A.gradN(kd.ix),LiDAR.A.gradE(kd.ix),...
    LiDAR.B.B(kd.ix),LiDAR.B.northness(kd.ix),LiDAR.B.slope(kd.ix),LiDAR.B.gradN(kd.ix),LiDAR.B.gradE(kd.ix),...
    LiDAR.C.C(kd.ix),LiDAR.C.northness(kd.ix),LiDAR.C.slope(kd.ix),LiDAR.C.gradN(kd.ix),LiDAR.C.gradE(kd.ix),...
...
ML.predictors = ML.MV;
ML.distributors = MV;
ML.paramNames = {'\rho_{obs}', 'constant', 'Hs','aspctHs','sxHs','dyHs','dxHs',...
...
ML.MLR.spatialDensity = MV*betas;
% ML.spatialDensity = MV(:,coms{optOut(optIx,1)}(optOut(optIx,2),:))*betas;
ML.MLR.spatialDensity = reshape(ML.MLR.spatialDensity,size(LiDAR.A.A));
ML.MLR.coeff = betas;
...
% MLR
% hI = imagesc((Coords.X)./1000,(Coords.Y)./1000,ML.MLR.spatialDensity,'AlphaData',0.625); 
...
colormap(cmap); 
caxis([300,450]);
% caxis([quantile(ML.MLR.spatialDensity(:),[0.005,0.995])]);
